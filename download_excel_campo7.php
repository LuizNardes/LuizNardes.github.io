...
$myArr = array('Areas','Responsaveis', '1 Trimestre','2 Trimestre','3 Trimestre','4 Trimestre','Anual');
 
$excel->writeLine($myArr,'black');
include 'banco_configuracao/config.php';
$query = "SELECT * FROM NOTAS_AREAS WHERE setor='ADMINISTRACAO'";
$result = sqlsrv_query($conn,$query);
while($linha = sqlsrv_fetch_array($result)){
    $myArr=array(utf8_decode($linha['area']),$linha['responsavel8s'],$linha['nota_1_tri'],$linha['nota_2_tri'], $linha['nota_3_tri'],$linha["nota_4_tri"],$linha['nota_anual']);
         $excel->writeLine($myArr,'black');
     }
