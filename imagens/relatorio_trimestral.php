...
$Trimestre = $_GET["tri"];
$Lider = $_GET["nome"];
$sql= "select distinct a.responsavel_8s, a.nome_senso, a.area_industrial, a.valor, b.observacao, c.upload  from (
...
$retorno .= "<div style='border-left:1px solid black;border-right:1px solid black;border-bottom:1px solid black;width:100%;margin-left:18px;'>
				<table>
					<thead>
						<tr style='background-color:rgb(0,96,255);color:white'>
							<th style='color:white'>Id</th>
							<th style='color:white'>Data</th>
							<th style='color:white'>Área da NC</th>
							<th style='color:white'>Senso da NC</th>
							<th style='color:white'>Foto</th>
							<th style='color:white'>Comentário NC</th>
							<th style='color:white'>Auditor 8S</th>
						</tr>
					</thead>
					<tbody>";
$i = 0;
while($row = sqlsrv_fetch_array($output)){
    $retorno .= "<tr style='border-top:1px solid black'><td>".$i."</td><td>".$row["data"]."</td><td>".$row["area_industrial"]."</td><td>".$row["nome_senso"]."</td><td><img src='".$row["upload"]."' style='width:25%'></td><td>".$row["observacao"]."</td><td>".$row["auditor"]."</td></tr>";
    $i++;
}
$retorno .= "</tbody></table>
			</div>";
